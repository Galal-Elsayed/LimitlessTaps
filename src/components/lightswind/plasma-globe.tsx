...
vec3 vmarch(vec3 ro, vec3 rd, float j, vec3 orig, float t){
  ...
  for (int i = 0; i < VOLUMETRIC_STEPS; i++){
    ...
    // attenuate with distance from origin and add - increased brightness
...
void main(){
  ...
  // Start with zero color (will be transparent where no plasma)
  vec3 col = vec3(0.0);
  ...
  for (float j = 1.0; j < NUM_RAYS + 1.0; j++){
    ...
    if (rz >= FAR) continue;
    vec3 pos = ro + rz * rd;
    vec3 rayCol = vmarch(pos, rd, j, bro, uTime);
    col = max(col, rayCol);
    brightness = max(brightness, dot(rayCol, vec3(0.299, 0.587, 0.114)));
  }
  ...
  if (sph.x > 0.0){
    ...
    vec3 sphereCol = (0.1 * nz * nz * vec3(0.12, 0.12, 0.5) + 0.05 * nz2 * nz2 * vec3(0.55, 0.2, 0.55)) * 1.8;
    col += sphereCol;
    brightness = max(brightness, dot(sphereCol, vec3(0.299, 0.587, 0.114)) * 2.0);
  }
  ...
  // Calculate alpha based on brightness - transparent where no plasma
  float alpha = clamp(brightness * 3.0, 0.0, 1.0);
  fragColor = vec4(col, alpha);
}
`;

// Mobile-optimized shader - reduced complexity for better performance
const FRAGMENT_SHADER_MOBILE = `#version 300 es
