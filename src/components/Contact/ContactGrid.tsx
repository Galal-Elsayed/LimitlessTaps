...
const ContactCard = ({
  ...
  >
    ...
    <div
      className={`absolute inset-0 opacity-0 group-hover:opacity-10 transition-opacity duration-700 bg-linear-to-br ${color} blur-3xl -z-10`}
    />

    <div
      className={`relative z-10 p-3 md:p-4 rounded-2xl md:rounded-3xl bg-white/5 border border-white/5 transition-all duration-500 group-hover:scale-110 ${iconColor}`}
    >
    ...
    <div className="relative z-10">
      <h3
        className={`font-bold text-white tracking-tight transition-colors ${isLarge ? "text-xl md:text-2xl" : "text-sm md:text-lg"}`}
      >
        {title}
      </h3>
      {description && (
    ...
    <div
      className={`absolute top-6 right-6 opacity-0 group-hover:opacity-100 group-hover:translate-x-1 group-hover:-translate-y-1 transition-all duration-500 ${isRtl ? "left-6 right-auto -scale-x-100" : ""}`}
    >
...
export default function ContactGrid() {
  ...
  // Actually, let's use useLocale from next-intl
