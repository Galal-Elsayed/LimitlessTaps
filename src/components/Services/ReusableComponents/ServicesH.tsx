...
export default function ServicesH({
  ...
  return (
    <section
      className={`relative bg-[#0a0a0a] text-[#bfbfbf] w-full ${height} flex items-start justify-start overflow-hidden perspective-2000`}
    >
      ...
      <div className="relative z-10 w-full max-w-[1800px] px-4 md:px-12 pointer-events-none flex flex-col justify-start h-full pt-32 lg:pt-18">
        ...
        >
          <h2
            className={`font-medium uppercase tracking-[-0.02em] font-sans text-white/90 mix-blend-difference mb-8 ${headerClassName}`}
          >
            {header}
          </h2>
    ...
    >
      ...
      <div className="relative w-full h-full duration-300 ease-out transition-all transform-style-3d group-hover:-translate-y-2 group-hover:translate-z-10 group-hover:rotate-x-2 group-hover:shadow-2xl">
        ...
        <div className="absolute inset-0 bg-[#0a0a0a] flex items-center pl-8 md:pl-12 transition-colors duration-300 group-hover:bg-[#0f0f0f] border border-transparent group-hover:border-[#333]/30">
          ...
          {!hidden && (
            <span className="text-xl md:text-2xl font-light tracking-wide text-[#444] group-hover:text-white transition-colors duration-300 uppercase z-10 relative select-none">
              {name}
            </span>
          )}
        ...
        <div className="absolute inset-x-0 -bottom-2 h-2 bg-[#0a0a0a] origin-top transform rotate-x-90 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      </div>
    </motion.div>
  );
}
